...
#include <stdio.h>
#include <string.h> // used for printing output
...
/*         fault_position specifies where the Eroor Locator Polynomial need to be attacked, -1 no injection ,position of fault injection in locator matrix (one column is one coeffient in GF(2^m))*/
/*          fault_value, specifies which bit(s) are compromised by fault injection */
/* output: e, error vector */
/* return: 0 for success; 1 for failure */
int decrypt_faulty(unsigned char *e, const unsigned char *sk, const unsigned char *s, const unsigned char fault_position, short unsigned int fault_value)
{
	int i; 
...
// 	for (i=0;i<GFBITS;i++){
//         uint64_t v64val[2];
//         memcpy(v64val, &locator[i], sizeof(v64val));
//         printf("%.16lx %.16lx\n", v64val[1], v64val[0]);
//     }
	
	uint64_t fault[2]={0,0};
	// bit 0 of all is at locator[i], degree 0 of ELP is the highest bit of uint64
	if(fault_position<SYS_T){
		 for (i=0;i<GFBITS;i++){
			 if(fault_position>64){
				 fault[1]=(((uint64_t)fault_value >>i)&1)<< (fault_position-64);
			}else{
				fault[0]=(((uint64_t)fault_value >>i)&1)<< (fault_position);
			}
			 // BIT RESET
             locator[i]=vec128_or(locator[i],vec128_set2x(fault[0],fault[1]));
             locator[i]=vec128_xor(locator[i],vec128_set2x(fault[0],fault[1]));
