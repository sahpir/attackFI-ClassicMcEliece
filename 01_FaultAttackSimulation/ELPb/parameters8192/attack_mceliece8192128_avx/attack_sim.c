...
int main()
{
    ...
    uint64_t found[128]; // works only if SYS_N=8192; stores which different support elements are already found
    ...
    for(i=0;i<128;i++) // works only if SYS_N=8192
        found[i]=0;
    ...
    for (int num=0; num<KATNUM; num++){
        ...
        /*fp_support = fopen("061550_mceliece8192_support.txt", "r");
        if (!fp_support)
            return FILE_OPEN_ERROR;
...
//                    printf("WRONG EQ");
                ...
                // checks if equations contain all 8192 support eleements
                found_all=1;                
        ...
        e_weight=2;
        fault_position=SYS_T-e_weight+fault_degree; // Calculated using fault_degree and e_weight, gives where to inject in locator
        fault_value=0b0000000000011; // bit 0 here represents bit 0 of a coefficient in ELP
