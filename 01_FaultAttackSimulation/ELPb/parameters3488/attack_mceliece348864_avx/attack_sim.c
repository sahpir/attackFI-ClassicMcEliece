...
int get_indizes_weight2(uint16_t* faulty_indizes, unsigned char * output_faulty_e, uint16_t zero_position);
...
int main()
{
    ...
    uint64_t found[55]; // works only if SYS_N=3488; stores which different support elements are already found
    ...
    for(i=0;i<55;i++) // works only if SYS_N=3488
        found[i]=0;
    ...
    for (int num=0; num<KATNUM; num++){
        ...
        /*fp_support = fopen("061550_mceliece3488_support.txt", "r");
        if (!fp_support)
            return FILE_OPEN_ERROR;
...
//             printf("%.4x ",g[i]);
       ...
       printf("\nL in support: \n");
        ...
        unsigned char nr_adjacent_setbits=2; // Must be smaller than GFBITS
        fault_value=(1<<nr_adjacent_setbits)-1;
        unsigned int distance=0; // If all m-(nr_adjacent_setbits-1) possible fault_values are reached, a new distance of set bits in plaintexts need to be chosen e.g. if distance =0 then (N-1,0),(0,1), (1,2),.. if distance=1 then (N-1,1),(3,5),()...
        
#if DEBUG_LEVEL > 2
        printf("fault_value: %d\n",fault_value);
#endif
        
        while(nr_found_variables < SYS_N || nr_found_equations < SYS_N){ // needed if all variables are searched
            k=0;
            indizes[0]=SYS_N-1; 
            indizes[1]=(k+distance)%SYS_N; // start with indizes (N-1,0)
            
            while(k <SYS_N && (nr_found_variables < SYS_N || nr_found_equations < SYS_N )){
                ...
                faulty_weight=get_indizes_weight2(faulty_indizes, output_faulty_e, zero_position); //  check if faulty e' has weight 2 (without zero position) 
...
#endif                    
                if(faulty_weight==2 && (indizes[0]!=faulty_indizes[0] || indizes[1]!= faulty_indizes[1] )
                    && !(indizes[0]==faulty_indizes[1] && indizes[1]==faulty_indizes[0])){
                            
                    // Check equations
//                    if((L[indizes[0]]^L[indizes[1]]^L[faulty_indizes[0]]^L[faulty_indizes[1]]) !=0)
//                    printf("WRONG EQ");
                            
                    fprintf(fp_sage_lin,"%d %d %d %d\n",indizes[0],indizes[1],faulty_indizes[0],faulty_indizes[1]);
                    ...
                    found[faulty_indizes[1]/64]=found[faulty_indizes[1]/64]|((uint64_t)1<<(faulty_indizes[1]%64));
                ...
                // checks if equations contain all 3488 support eleements
                found_all=1;                
                ...
                uint64_t result= found[54];
                for (int j = 0; j < 32; j++){
                    nr_found_variables += (result & 1);
                    result = (result >> 1); 
                }
                //Checks last 32 variables
                if(found[54]!=0x00000000ffffffff)
                    found_all=0;
                ...
                indizes[0]=k; indizes[1]=(k+1+distance)%SYS_N;
            ...
            nr_loops_constant++;
            if(nr_loops_constant%(GFBITS-nr_adjacent_setbits+1)==0){ // Fault value is reset if distance is increased
                distance++;
                fault_value=(1<<nr_adjacent_setbits)-1; 
            }
...
#endif
#if DEBUG_LEVEL >2              
            // prints which variables are already found
            printf("\n nr %d\n", nr_found_variables);
            for(i=0;i<55;i++)  
                printf("%016lx ", found[i]);
#endif
        }//end while nr_equations
        ...
        e_weight=2;
        fault_position=SYS_T-e_weight+fault_degree; // Calculated using fault_degree and e_weight, gives where to inject in locator
        fault_value=0b000000000011; // bit 0 here represents bit 0 of a coefficient in ELP
        unsigned int nr_requested_lineq=80;
        
        
        // Does linear injections until nr_requested_lineq equations are found
        while(nr_found_equations-nr_found_constant_equations<nr_requested_lineq){
            k=0;
            indizes[0]=SYS_N-1;
            indizes[1]=k; // start with indizes (N-1,0)
            
            while(k <SYS_N && (nr_found_equations-nr_found_constant_equations<nr_requested_lineq)){
                gen_e_modified(e,e_weight, indizes); // construct e with e_weight at indizes positions
                ...
                // for simulation purposes use this to verify ouput of parallelized brute force for finding e' 
                decrypt_faulty(output_faulty_e, sk +40, ct, fault_position,fault_value);
                faulty_weight=get_indizes_weight2(faulty_indizes, output_faulty_e, zero_position);

                    
                if(faulty_weight==2 && (indizes[0]!=faulty_indizes[0] || indizes[1]!= faulty_indizes[1]) && !(indizes[0]==faulty_indizes[1] && indizes[1]==faulty_indizes[0])){      
...
//                 }
        ...
        /************* END: LINEAR INJECTIONS (Quadratic Equations) ***************/
 ...
 * Output: positions/indizes of faulty error vector if Hamming weight is 2
...
int get_indizes_weight2(uint16_t* faulty_indizes, unsigned char * output_faulty_e, uint16_t zero_position){
    ...
    if(found==3 && faulty_indizes[1]==zero_position){
        faulty_indizes[1]=faulty_indizes[2];
        found--;
    }
    if(found==3 && faulty_indizes[2]==zero_position)
        found--;
    if(found==3 && faulty_indizes[0]==zero_position){
        faulty_indizes[0]=faulty_indizes[1];
        faulty_indizes[1]=faulty_indizes[2];
        found--;
    }
    //remove all equations including the zero_position, because it messes up the polynomial system
    if(faulty_indizes[0]==zero_position || faulty_indizes[1]==zero_position){
        found --;
    }
