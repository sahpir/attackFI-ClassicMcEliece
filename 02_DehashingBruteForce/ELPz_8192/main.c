...
//     printf("filename_input: %s\n", parameters->filename_input);
    ...
    //sprintf(filename_output_thread,"%s",parameters->filename_output_thread);
    //printf("parameters->filename_output_thread: %s\n", parameters->filename_output_thread);
    //printf("filename_output_thread: %s\n",filename_output_thread);
    fp_output = fopen(parameters->filename_output_thread, "w");
    //fp_output = fopen(filename_output_thread, "w");
    if (!fp_output)
...
//             printf("Written nr of characters: %d\n",fprintf(fp_output,"%d %d %d %d\n",element.indizes[0],element.indizes[1],faulty_indizes[0],faulty_indizes[1]));
    ...
    // argv order: program name, "lin" or "quad", filename_input, filename_output, (zero_position)
